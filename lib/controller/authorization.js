...
module.exports = function (req, res, next) {
  ...
  var clientId, redirectUri, responseType, grantType, client, scope, user;
    ...
    [
        ...
        if (!req.query.redirect_uri)
          return cb(
            new error.invalidRequest(
              "RedirectUri is mandatory for authorization endpoint"
            )
          );
      ...
      function (cb) {
        if (!req.query.client_id)
          return cb(
            new error.invalidRequest(
              "ClientId is mandatory for authorization endpoint"
            )
          );
        ...
        if (req.query.client_secret)
          return cb(
            new error.invalidRequest(
              "ClientSecret should not be passed by public clients"
            )
          );
      ...
      function (cb) {
        if (!req.query.response_type)
          return cb(
            new error.invalidRequest(
              "ResponseType parameter is mandatory for authorization endpoint"
            )
          );

        responseType = req.query.response_type;
        req.oauth2.logger.debug(
          "Parameter response_type parsed: ",
          responseType
        );
        cb();
      },
      ...
      function (cb) {
        switch (responseType) {
          ...
          default:
            return cb(
              new error.unsupportedResponseType(
      ...
      function (cb) {
        req.oauth2.model.client.fetchById(clientId, function (err, obj) {
          if (err)
            cb(
              new error.serverError("Failed to call client::fetchById method")
            );
          else if (!obj) cb(new error.invalidClient("Client not found"));
          else {
      ...
      function (cb) {
        if (!req.oauth2.model.client.getRedirectUri(client))
          cb(
            new error.unsupportedResponseType(
              "RedirectUri is not set for the client"
            )
          );
      ...
      function (cb) {
        if (!req.oauth2.model.client.checkGrantType(client, grantType))
          cb(
            new error.unauthorizedClient(
              "Grant type is not available for the client"
            )
          );
        else {
      ...
      function (cb) {
        ...
        if (!scope) cb(new error.invalidScope("Invalid scope for the client"));
        else {
      ...
      function (cb) {
        ...
        if (!user)
          cb(
            new error.invalidRequest(
              "Failed to fetch logged user from request parameters"
            )
          );
        else {
      ...
      if (err) response.error(req, res, err, redirectUri);
        ...
        if (grantType == "authorization_code")
          ...
          response.error(
            req,
            res,
            new error.invalidRequest("Wrong request method"),
            redirectUri
          );
      }
    }
